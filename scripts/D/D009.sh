...
python3 -m polyspace.train.train_converter \
	...
	--loss_bar 0.1 \
	--loss_l1 0.0 \
	--save_dir ./checkpoints/D009/converter >> D009.log 2>&1

python3 -m polyspace.train.train_fusion \
    --features ./features/diving48/features_diving48_train.index.json \
    --teachers videomaessv2 timesformerssv2 vivit \
    --converters ./checkpoints/D009/converter/converters_ep10.pt \
    --classes 48 \
    --batch 8 \
    --epochs 15 \
    --lr 3e-4 \
    --features_fp16 \
    --save_dir ./checkpoints/D009/fusion >> D009.log 2>&1

for ep in {1..15}; do
  echo "checkpoint: ep$ep" >> D009E.log 2>&1
  python3 -m polyspace.train.eval_downstream \
    --features ./features/diving48/features_diving48_test.index.json \
    --teachers videomaessv2 timesformerssv2 vivit \
    --converters ./checkpoints/D009/converter/converters_ep10.pt \
    --fusion ./checkpoints/D009/fusion/fusion_ep$ep.pt \
    --features_fp16 >> D009E.log 2>&1
done
