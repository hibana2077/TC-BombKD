...
python3 -m polyspace.train.train_converter \
	...
	--loss_l1 0.0 \
  --subsample_ratio 0.1 \
	--save_dir ./checkpoints/DS005/converter >> DS005.log 2>&1

python3 -m polyspace.train.train_fusion \
    --features ./features/diving48/features_diving48_train.index.json \
    --teachers videomaessv2 timesformerssv2 vivit \
    --converters ./checkpoints/DS005/converter/converters_ep10.pt \
    --classes 48 \
    --batch 8 \
    --epochs 50 \
    --lr 3e-4 \
    --features_fp16 \
    --subsample_ratio 0.1 \
    --save_dir ./checkpoints/DS005/fusion >> DS005.log 2>&1

for ep in {1..50}; do
  echo "checkpoint: ep$ep" >> DS005E.log 2>&1
  python3 -m polyspace.train.eval_downstream \
    --features ./features/diving48/features_diving48_test.index.json \
    --teachers videomaessv2 timesformerssv2 vivit \
    --converters ./checkpoints/DS005/converter/converters_ep10.pt \
    --fusion ./checkpoints/DS005/fusion/fusion_ep$ep.pt \
    --features_fp16 >> DS005E.log 2>&1
done
