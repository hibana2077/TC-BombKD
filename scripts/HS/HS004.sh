...
python3 -m polyspace.train.train_converter \
	...
	--loss_l1 0.0 \
    --subsample_ratio 0.1 \
	--save_dir ./checkpoints/HS004/converter >> HS004.log 2>&1

python3 -m polyspace.train.train_fusion \
	--features ./features/hmdb51/features_hmdb51_train.index.json \
	--teachers vivit videomaeg timesformerg \
	--converters ./checkpoints/HS004/converter/converters_ep10.pt \
	--classes 51 \
	--batch 4 \
	--epochs 50 \
	--lr 3e-4 \
	--features_fp16 \
    --subsample_ratio 0.1 \
	--save_dir ./checkpoints/HS004/fusion >> HS004.log 2>&1

for ep in {1..50}; do
  echo "checkpoint: ep$ep" >> HS004.log 2>&1
	python3 -m polyspace.train.eval_downstream \
		--features ./features/hmdb51/features_hmdb51_test.index.json \
		--teachers vivit videomaeg timesformerg \
		--converters ./checkpoints/HS004/converter/converters_ep10.pt \
		--fusion ./checkpoints/HS004/fusion/fusion_ep$ep.pt \
		--features_fp16 >> HS004E.log 2>&1
done
