...
python3 -m polyspace.train.train_converter \
	...
	--loss_bar 0.00 \
	--loss_l1 0.00 \
	--save_dir ./checkpoints/H022/converter >> H022.log 2>&1

python3 -m polyspace.train.train_fusion \
	--features ./features/hmdb51/features_hmdb51_train.index.json \
	--teachers vivit videomaeg timesformerg \
	--converters ./checkpoints/H022/converter/converters_ep10.pt \
	--classes 51 \
	--batch 8 \
	--epochs 40 \
	--lr 3e-4 \
	--features_fp16 \
	--advance-cls-head \
	--save_dir ./checkpoints/H022/fusion >> H022.log 2>&1

for ep in {1..40}; do
  echo "checkpoint: ep$ep" >> H022.log 2>&1
	python3 -m polyspace.train.eval_downstream \
		--features ./features/hmdb51/features_hmdb51_test.index.json \
		--teachers vivit videomaeg timesformerg \
		--converters ./checkpoints/H022/converter/converters_ep10.pt \
		--fusion ./checkpoints/H022/fusion/fusion_ep$ep.pt \
		--advance-cls-head \
		--features_fp16 >> H022E.log 2>&1
done
