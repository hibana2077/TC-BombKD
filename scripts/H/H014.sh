...
python3 -m polyspace.train.train_converter \
	...
	--loss_bar 0.0 \
	--loss_l1 0.0 \
	--save_dir ./checkpoints/H014/converter >> H014.log 2>&1

python3 -m polyspace.train.train_fusion \
	--features ./features/hmdb51/features_hmdb51_train.index.json \
	--teachers vivit videomaeg timesformerg \
	--converters ./checkpoints/H014/converter/converters_ep10.pt \
	--classes 51 \
	--batch 8 \
	--epochs 50 \
	--lr 3e-4 \
	--features_fp16 \
	--save_dir ./checkpoints/H014/fusion >> H014.log 2>&1

for ep in {1..50}; do
  echo "checkpoint: ep$ep" >> H014.log 2>&1
	python3 -m polyspace.train.eval_downstream \
		--features ./features/hmdb51/features_hmdb51_test.index.json \
		--teachers vivit videomaeg timesformerg \
		--converters ./checkpoints/H014/converter/converters_ep10.pt \
		--fusion ./checkpoints/H014/fusion/fusion_ep$ep.pt \
		--features_fp16 >> H014E.log 2>&1
done
